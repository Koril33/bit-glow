...
def generate_ascii_c_array(width=8, height=16, font_path="arial.ttf", threshold=128):
    ...
    ascii_array = f"const unsigned char ascii_{width}x{height}[{128}][{width * 2}] = " + '{\n'
...
def generate_chinese_c_array(chinese='你好', width=16, height=16, font_path="simhei.ttf", threshold=128):
    ...
    chinese_array = f"const unsigned char chinese_{width}x{height}[{len(chinese)}][{width * 4}] = {{\n"
...
def main():
    ...
    width = 8
    height = 16
    font_path = '/usr/share/fonts/truetype/win/SIMSUN.ttf'
    c_array = generate_ascii_c_array(
        width=width, height=height,
        font_path=font_path,
    )
    # c_array = generate_chinese_c_array(chinese='梁芝琴', width=width, height=height, font_path=font_path)
    print(c_array)
    #
